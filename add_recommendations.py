...
RECOMMEND_JS = """
    ...
    function loadRecommendations() {
        ...
        const currentMovie = document.querySelector('.title') ? document.querySelector('.title').textContent : '';
        
        fetch('movie_recommendations.json')
            .then(response => response.json())
            .then(data => {
                const recs = data[currentMovie] || [];
                if (recs.length === 0) {
...
def process_detail_files():
    ...
    for filename in sorted(detail_files):
        ...
        print(f"处理: {movie_title} ({filename})")
    ...
    print(f"\n完成！更新了 {updated} 个文件，跳过 {skipped} 个已有推荐模块的文件")
